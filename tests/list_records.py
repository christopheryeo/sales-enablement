...
def list_all_records():
    ...
    try:
        ...
        select_sql = f"SELECT id, email, timestamp, \"Organisation\", sections_completed, quizzes_completed, last_active FROM {TABLE_NAME} ORDER BY timestamp DESC;"
        ...
        if records:
            ...
            for record in records:
                record_id, email, timestamp, org, sections, quizzes, last_active = record
                print(f"ID: {record_id}")
                print(f"Email: {email}")
                print(f"Organisation: {org}")
                print(f"Timestamp: {timestamp}")
                print(f"Sections Completed: {sections}")
                print(f"Quizzes Completed: {quizzes}")
